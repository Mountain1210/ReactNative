...
class List extends Component {
  ...
  componentDidMount(){
    var _that = this;
    console.log(AsyncStorage.getAllKey)
    // AsyncStorage.getAllKey.bind(_that, function(err, keys){
  ...
  render() {
    ...
    for(var i in Model){
      if(i % 2 === 0){
        var row = (
          <View style={styles.row} key={Model[i].id}>
...
class MyProject extends Component {
  render() {
    ...
    renderNav(route,nav){
        switch (route.title) {
          case '水果列表':
            return <List title={route.title} navigator={navigator}/ >;
          case '购物车':
            return (<GouWu title={route.title} navigator={navigator}/>);
        }
    }
}
